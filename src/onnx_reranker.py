...
class OnnxReranker:
    ...
    def __init__(self, model_dir: str | Path, model_file: str, max_length: int = 8192):
        ...
        self.model_max_length = getattr(self.config, "max_position_embeddings", 32768)
    ...
    def prepare_inputs(
        self, queries: Iterable[str], documents: Iterable[str], instruction: Optional[str] = None
    ) -> dict:
        inputs = []
    ...
    def scores(self, query: str, documents: List[str], instruction: Optional[str] = None) -> List[float]:
        payload = self.prepare_inputs([query] * len(documents), documents, instruction=instruction)
        logits = self.session.run(None, payload)[0]
    ...
    def rerank(self, request: RerankRequest) -> List[dict]:
        scores = self.scores(request.query, request.documents, request.instruction)
        ranked = sorted(zip(request.documents, scores), key=lambda x: x[1], reverse=True)[: request.top_n]
